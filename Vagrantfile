...
Vagrant.configure(2) do |config|
    ...
    config.vm.define "full" do |guest|
        ...
        # Jira
        guest.vm.network "forwarded_port", host: 8080, guest: 8080

        # Confluence
        guest.vm.network "forwarded_port", host: 8090, guest: 8090

        # Bamboo
        guest.vm.network "forwarded_port", host: 8085, guest: 8085

        # Bitbucket
        guest.vm.network "forwarded_port", host: 7990, guest: 7990
        guest.vm.network "forwarded_port", host: 8006, guest: 8006

        # Crowd
        guest.vm.network "forwarded_port", host: 8095, guest: 8095

        # PostgreSQL
        guest.vm.network "forwarded_port", host: 5432, guest: 5432
    end
